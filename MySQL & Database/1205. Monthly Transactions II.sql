...
#2.combine ordinary table and temporary table to build a source table for filtering
(select * from Transactions t

#3.filtering records based on conditions 
where state='approved' union 

#1.create a temporary table to identity state of 'chargebacks'
select t.id, t.country, 'chargeback', t.amount, c.trans_date from Transactions t join Chargebacks c
on t.id=c.trans_id) as temp
group by 1,2; #each month and country
